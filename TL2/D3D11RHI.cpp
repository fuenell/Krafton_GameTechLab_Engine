...
void D3D11RHI::Release()
{
	...
	if (DepthStencilStateTestOnWriteOff) { DepthStencilStateTestOnWriteOff->Release(); DepthStencilStateTestOnWriteOff = nullptr; }
...
void D3D11RHI::CreateDepthStencilState()
{
	...
	Device->CreateDepthStencilState(&desc, &DepthStencilStateTestOnWriteOff);
...
void D3D11RHI::UpdateDecalConstantBuffers(const FMatrix& InVPMatrix)
{
	D3D11_MAPPED_SUBRESOURCE mapped;
	DeviceContext->Map(DecalCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped);
	auto* dataPtr = reinterpret_cast<FMatrix*>(mapped.pData);

	//dataPtr가 가리키는 메모리에 VPMatrix의 내용을 복사
	*dataPtr = InVPMatrix;
...
void D3D11RHI::CreateConstantBuffer()
{
	...
	// b6 : 데칼용 상수 버퍼
	D3D11_BUFFER_DESC DecalCBDesc = {};
	DecalCBDesc.Usage = D3D11_USAGE_DYNAMIC;
	DecalCBDesc.ByteWidth = sizeof(FMatrix);
	DecalCBDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	DecalCBDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	Device->CreateBuffer(&DecalCBDesc, nullptr, &DecalCB);
...
void D3D11RHI::OmSetDepthStencilState(EComparisonFunc Func)
{
	...
	case EComparisonFunc::GreaterEqual:
		...
		break;
	case EComparisonFunc::Decal:
		DeviceContext->OMSetDepthStencilState(DepthStencilStateTestOnWriteOff, 0);
		break;
	}
}
